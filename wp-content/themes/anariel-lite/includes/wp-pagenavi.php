...
function anariel_wp_pagenavi($before = '', $after = '') {
	...
	if ($max_page > 1 || intval($pagenavi_options['always_show'])) {
		$pages_text = str_replace("%CURRENT_PAGE%", number_format_i18n($paged), __('Page %CURRENT_PAGE% of %TOTAL_PAGES%','anariel'));
		...
		switch(intval($pagenavi_options['style'])) {
			...
			case 1:
				...
				if ($start_page >= 2 && $pages_to_show < $max_page) {
					$first_page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), __('&laquo; First','anariel'));
				...
				previous_posts_link(__('&laquo;','anariel'));
				...
				next_posts_link(__('&raquo;','anariel'), $max_page);
				$larger_page_end = 0;
				foreach($larger_pages_array as $larger_page) {
				...
				if ($end_page < $max_page) {
					...
					$last_page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), __('Last &raquo;','anariel'));
